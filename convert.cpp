...
void convert_to_gguf(TensorMap& tensors, ConvertParams& params) {
    ...
    size_t clip_tensors_size = 0, unet_tensors_size = 0, vae_tensors_size = 0;
    ...
    for (Tensor& tensor : processed_tensors) {
        ...
        void* dest   = NULL;
        if (params.verbose) {
            printf("%4s => %4s | %s\n", ggml_type_name(tensor.dtype), ggml_type_name(dest_type), tensor.name.c_str());
        }
        ...
        if (tensor.target == CLIP) {
            num_clip_tensors++;
            clip_tensors_size += ggml_nbytes(gg_tensor);
        } else if (tensor.target == UNET) {
            num_unet_tensors++;
            unet_tensors_size += ggml_nbytes(gg_tensor);
        } else if (tensor.target == VAE) {
            num_vae_tensors++;
            vae_tensors_size += ggml_nbytes(gg_tensor);
        }
    }
    ...
    printf("\nModel Tensors | Count | Size\n");
    printf("CLIP          | %5i | %.2f MB\n", num_clip_tensors, clip_tensors_size / 1024.0 / 1024.0);
    printf("UNET          | %5i | %.2f MB\n", num_unet_tensors, unet_tensors_size / 1024.0 / 1024.0);
    printf("VAE           | %5i | %.2f MB\n", num_vae_tensors, vae_tensors_size   / 1024.0 / 1024.0);
    printf("Total         | %5i | %.2f MB\n", num_clip_tensors + num_unet_tensors + num_vae_tensors, total_conv_model / 1024.0 / 1024.0);
...
void load_tensors_from_model(std::string path, TensorMap& tensors, ConvertParams& params, bool root_model, TensorTarget target = NONE) {
    ...
    if (
        buffer_[8] == '{' && (file_size <= 0 || (safe_tensor_metadata_size > 0 &&
        safe_tensor_metadata_size < (uint64_t)file_size))) {  // begin safetensor metadata
